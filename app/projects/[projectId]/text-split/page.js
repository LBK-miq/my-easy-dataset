...
export default function TextSplitPage({ params }) {
  ...
  // 处理生成问题
  const handleGenerateQuestions = async (chunkIds, type) => {
    try {
      var t1 = Date.now();
      ...
      if (chunkIds.length === 1) {
        ...
        const response = await request(`/api/projects/${projectId}/chunks/${encodeURIComponent(chunkId)}/questions`, {
          ...
          body: JSON.stringify({ model, language: currentLanguage, type: type })
        });
        ...
        const data = await response.json();
        console.log(t('textSplit.questionsGenerated', { chunkId, total: data.total, type: type }));
        setError({
          severity: 'success',
  ...
  // const handleGenerateDetailQuestions = async chunkIds => {
  //   try {
  //     var t1 = Date.now();
  //     console.log("开始生成问题（细粒度），毫秒数：", t1)
  //     setProcessing(true);
  //     setError(null);
  ...
  //           'Content-Type': 'application/json'
  //         },
  //         body: JSON.stringify({ model, language: currentLanguage, type: 'detail' })
  //       });
  ...
  // const handleGenerateGeneralQuestions = async chunkIds => {
  //   try {
  //     var t1 = Date.now();
  //     console.log("开始生成问题（粗粒度），毫秒数：", t1)
  //     setProcessing(true);
  //     setError(null);
  ...
  //           'Content-Type': 'application/json'
  //         },
  //         body: JSON.stringify({ model, language: currentLanguage, type: 'general' })
  //       });
      ...
      <Box sx={{ width: '100%', mb: 3 }}>
        ...
        {activeTab === 0 && (
          <ChunkList
            ...
            onGenerateDetailQuestions={handleGenerateQuestions}
            onGenerateGeneralQuestions={handleGenerateQuestions}
            loading={loading}
