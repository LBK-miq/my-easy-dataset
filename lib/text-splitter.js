...
import splitTextPrompt from '@/lib/llm/prompts/split';
 ...
 * @param model - 调用模型（目前改为大模型语义分段）
 * @returns {Promise<Array>} - 分割结果数组
 */
export async function splitProjectFile(projectId, fileName, model) {
  try {
    // 获取项目根目录
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      console.log(projectId, fileName, 'Try spliting text by LLM...');
      const promptFunc = splitTextPrompt;
      const prompt = promptFunc({ text: fileContent, min: minLength, max: maxLength });
      const response = await llmClient.getResponse(prompt);
      const LLMResponse = extractJsonFromLLMOutput(response);
      ...
      const splitResult = markdownSplitter.splitMarkdownWithLLM(fileContent, LLMResponse);
