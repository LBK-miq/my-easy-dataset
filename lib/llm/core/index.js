...
class LLMClient {
  ...
  _makeHttpRequest(url, options) {
    return new Promise((resolve, reject) => {
      ...
      var requestOptions;
      var req;
      if (urlObj.hostname == 'api.openai.com') {  //  改用request库
        requestOptions = {
          // rejectUnauthorized: false,
          url: url,
          method: options.method,
          headers: options.headers,
          body: options.body,                     //  指定body就好了，但是http/https库的调用不用指定body，是不同库原理不太一样的原因？
          proxy: 'http://127.0.0.1:7897'          //  梯子的代理端口
        };
        req = request(requestOptions, (err, response, body) => {
          const code = response && response.statusCode
          try {
            if (code >= 200 && code < 300) {
              resolve(JSON.parse(body))
            } else {
              reject(new Error(`请求失败，状态码: ${code}, 响应: ${body}`))
            } 
          } catch (error) {
            reject(new Error(`响应解析失败，原因：${error}\n源数据：${body}`));
          }
        });
      } else {  //  墙内使用http/https库
        requestOptions = {
          hostname: urlObj.hostname,
          port: urlObj.port || (isHttps ? 443 : 80),
          path: `${urlObj.pathname}${urlObj.search}`,
