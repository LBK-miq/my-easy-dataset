...
function getDetailQuestionPrompt({
    ...
    return `
      ...
      ## 核心任务
      根据用户提供的文本（长度：${text.length} 字），生成不少于 ${number} 个关注文本具体细节的高质量问题。
  
      ## 约束条件（重要！）
      - 必须基于文本内容直接生成
      - 问题应明确指向文本中的某个具体细节（如概念细节、具体规定等）
      - 需覆盖文本的不同方面
      - 禁止生成假设性、重复或相似问题
  
      ## 处理流程
      1. 【文本解析】分段处理内容，识别关键实体和核心概念
      2. 【问题生成】基于信息密度、文本细节选择最佳提问点
      3. 【质量检查】确保：
         - 问题答案可在原文中找到依据
         - 问题内容指向文本的细节具体描述
         - 无格式错误
...
function getGeneralQuestionPrompt({
    ...
    return `
      ...
      ## 约束条件（重要！）
      - 必须基于文本内容直接生成，即问题应在文中有较明确的答案
      - 问题应关注文本的整体概括、知识框架、上下文理解，提出粗粒度、总结性的问题，不直接针对某一具体知识点提问
      - 需覆盖文本的不同方面
      - 禁止生成假设性、重复或相似问题
