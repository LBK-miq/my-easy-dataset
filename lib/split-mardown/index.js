...
function splitMarkdownWithLLM(markdownText, LLMResponse) {
  ...
  try { 
    for (let i = 0; i < LLMResponse.length; i++) {
      ...
      var abstract = '';
      const abstractList = LLMResponse[i]['abstract'];
      for (let i = 0; i < abstractList.length; i++) {
        abstract += `(${i + 1}) ${abstractList[i]}\n\n`;
      }
      abstract = abstract.substring(0, abstract.length - 1)

      res.push({summary: LLMResponse[i]['topic'], abstract: abstract, content: splitTexts[0] + LLMResponse[i]['point']});
      waitingText = '';
      for (let j = 1; j < splitTexts.length; j++) {
        waitingText += splitTexts[j];
      }
    }
    res.push({summary: '全文内容', abstract: '无', content: markdownText});
  } catch (error) {
