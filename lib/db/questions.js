...
export async function addQuestionsForChunk(projectId, chunkId, newQuestions) {
  ...
  const type = newQuestions[0]['label']

  if (existingIndex >= 0) {
    // 更新现有问题（粗粒度与细粒度不互相覆盖）
    var questionList = []
    for (let i = 0; i < questions[existingIndex].questions.length; i++) {
      if (questions[existingIndex].questions[i]['label'] != type) {
        questionList.push(questions[existingIndex].questions[i])
      }
    }
    for (let i = 0; i < newQuestions.length; i++) {
      questionList.push(newQuestions[i])
    }
    questions[existingIndex].questions = questionList;
  } else {
    // 添加新问题
    questions.push({
      chunkId,
